...
// If we don't do this, the text and button coordinates will be off
  ...
  // Clear APs
    ...
    if (ss_sw == -1) {
      ...
      if (!result) {
        Serial.println("Could not successfully update setting \"" +
                       setting_name + "\"");
        return;
      }
    }
  ...
  if (!wifi_scan_obj.scanning()) {
    ...
    if (cmd_args.get(0) == SIGSTREN_CMD) {
      Serial.println("Starting Signal Strength Scan. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
    ...
    // Packet count
      ...
      Serial.println("Starting Packet Count Scan. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // Wardrive
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        if (sta_sw == -1) {
          Serial.println("Starting Wardrive. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
          ...
          Serial.println("Starting Station Wardrive. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
    ...
    // Karma
      ...
      if (evil_portal_obj.setAP(probe_req_ssids->get(pr_index).essid)) {
        Serial.println("Starting Karma Attack with " +
                       probe_req_ssids->get(pr_index).essid + ". Stop with " +
                       (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
        display_obj.clearScreen();
        menu_function_obj.drawStatusBar();
    ...
    // AP Scan
      ...
      Serial.println("Scanning for APs and Stations. Stop with " +
                     (String)STOPSCAN_CMD);
    ...
    // Scan stations
      ...
      if (access_points->size() < 1)
        Serial.println("The AP list is empty. Scan APs first with " +
                       (String)SCANAP_CMD);

      Serial.println("Starting Station scan. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // Beacon sniff
      ...
      Serial.println("Starting Beacon sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // Deauth sniff
      ...
      Serial.println("Starting Deauth sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // Pwn sniff
      ...
      Serial.println("Starting Pwnagotchi sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // PineScan sniff
      ...
      Serial.println("Starting Pinescan sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // MultiSSID sniff
      ...
      Serial.println("Starting MultiSSID sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    // Espressif sniff
      ...
      Serial.println("Starting Espressif device sniff. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
    ...
    if (cmd_args.get(0) == BT_SNIFF_CMD) {
      ...
      if (bt_type_sw != -1) {
        ...
        // Airtag sniff
        if (bt_type == "airtag") {
          Serial.println("Starting Airtag sniff. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
          ...
          Serial.println("Starting Flipper sniff. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
      ...
      // General bluetooth sniff
        ...
        Serial.println("Starting Bluetooth scan. Stop with " +
                       (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
        display_obj.clearScreen();
        menu_function_obj.drawStatusBar();
      ...
      if (at_sw != -1) {
        ...
        if (target_mac < airtags->size()) {
          ...
          Serial.println("Provided index is out of range: " +
                         (String)target_mac);
          return;
        }
#endif
      ...
      if (bt_type_sw != -1) {
        ...
        if (bt_type == "apple") {
#ifdef HAS_BT
          Serial.println("Starting Sour Apple attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
          Serial.println("Starting Swiftpair Spam attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
          Serial.println("Starting Samsung Spam attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
          Serial.println("Starting Google Spam attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
          Serial.println("Starting Flipper Spam attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
          Serial.println("Starting BT Spam All attack. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
    ...
    // Wardrive
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        if (cont_sw == -1) {
          Serial.println("Starting BT Wardrive. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
          ...
          Serial.println("Starting Continuous BT Wardrive. Stop with " +
                         (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
          display_obj.clearScreen();
          menu_function_obj.drawStatusBar();
...
#ifdef HAS_BT
      Serial.println("Starting Bluetooth CC Skimmer scan. Stop with " +
                     (String)STOPSCAN_CMD);
#ifdef HAS_SCREEN
      display_obj.clearScreen();
      menu_function_obj.drawStatusBar();
