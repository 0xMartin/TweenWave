...
void Buffer::add(const uint8_t *buf, uint32_t len, bool is_pcap) {
  ...
  uint32_t seconds = (microSeconds / 1000) /
                     1000; // e.g. 45200400/1000/1000 = 45200 / 1000 = 45s

  microSeconds -= seconds * 1000 *
                  1000; // e.g. 45200400 - 45*1000*1000 = 45200400 - 45000000 =
                        // 400us (because we only need the offset)
...
void Buffer::saveSerial() {
  ...
  // This is necessary so that other console output isn't mixed into buffer
  // stream
  uint8_t *buf =
      (uint8_t *)malloc(mark_begin_len + bufSizeA + bufSizeB + mark_close_len);
  uint8_t *it = buf;
